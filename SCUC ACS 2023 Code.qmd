...
# Method 1: Filter after pulling all districts
scuc_full_name1_5 <- tx_schools_5 %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_full_name1_5)


# Now, use ACS 1-year data 

scuc_full_name1_1 <- tx_schools_1 %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_full_name1_1)
...
scuc_full_name2_5 <- all_districts %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_full_name2_5)
...
# Search only for specific district full name (no variables)
scuc_full_name2_1 <- all_districts %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_full_name2_1)
...
scuc_multi_vars_5 <- tx_school_data_5 %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_multi_vars_5)



# Now, use 1 year data
tx_school_data_1 <- get_acs(
  geography = "school district (unified)",
  state = "TX",
  variables = c(
    total_pop = "B01003_001",           # Total population
...
# Filter for specific district
scuc_multi_vars_1 <- tx_school_data_1 %>%
  filter(str_detect(NAME, "Schertz"))

View(scuc_multi_vars_1)
...
scuc_no_diploma_with_geometry_5 <- get_acs(
  geography = "school district (unified)",
  state = "TX",
  variables = c(
...
View(scuc_no_diploma_with_geometry_5)


# Now, use 1-year data
scuc_no_diploma_with_geometry_1 <- get_acs(
  geography = "school district (unified)",
  state = "TX",
  variables = c(
    "B01003_001",     # Total population
...
View(scuc_no_diploma_with_geometry_1)
...
district_comprehensive_5 <- get_acs(
  geography = "school district (unified)",
  state = "TX",
  variables = c(
    # Demographics
    ...
    # Age groups
    under_5 = "B01001_003",
    age_5_17 = "B09001_001"  # School age population
  ),
  year = 2023,
  survey = "acs5"
...
View(district_comprehensive_5)


# Now using 1-year data
district_comprehensive_1 <- get_acs(
  geography = "school district (unified)",
  state = "TX",
  variables = c(
    # Demographics
...
education_vars_5 <- all_vars_5 %>%
  filter(str_detect(concept, regex("educational", ignore_case = TRUE)))


View(education_vars_5)



# Load all available 1-year variables to explore
all_vars_1 <- load_variables(2023, "acs1")

# Search for education-related variables
education_vars_1 <- all_vars_1 %>%
  filter(str_detect(concept, regex("educational", ignore_case = TRUE)))


View(education_vars_1)
...
median_home_value_1_guad <- get_acs(
  geography = "county",
  state = "TX",
  county = "Guadalupe",
  variables = "B25077_001",
