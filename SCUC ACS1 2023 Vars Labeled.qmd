---
title: "SCUC ACS1 2023 Variables"
format: html
editor: 
  markdown: 
    wrap: 72
...
# Sample of Variables Concerning SCUC-ISD available in ACS1 dataset
...
### Pull 2023 ACS1 Table B14001 (school enrollment data) for SCUC
...
# Save as RDS
saveRDS(scuc_enrollment_acs1_table_b14001_labeled, "scuc_data/scuc_enrollment_acs1_table_b14001_labeled.rds")

# # Load RDS later
# scuc_enrollment_acs1_table_b14001_labeled <- readRDS("scuc_data/scuc_enrollment_acs1_table_b14001_labeled.rds")


# Save as CSV
write_csv(scuc_enrollment_acs1_table_b14001_labeled, "scuc_data/scuc_enrollment_acs1_table_b14001_labeled.csv")

# # Load CSV later
# scuc_enrollment_acs1_table_b14001_labeled <- read_csv("scuc_data/sscuc_enrollment_acs1_table_b14001_labeled.csv")


library(writexl)

write_xlsx(scuc_enrollment_acs1_table_b14001_labeled, "scuc_data/scuc_enrollment_acs1_table_b14001_labeled.xlsx")
...
### Pull 2023 ACS1 Table B14002 (school enrollment by SEX) for SCUC
...
# Save as RDS
saveRDS(scuc_enrollment_by_sex_acs1_table_b14002_labeled , "scuc_data/scuc_enrollment_by_sex_acs1_table_b14002_labeled.rds")

# # Load RDS later
# scuc_enrollment_by_sex_acs1_table_b14002_labeled <- readRDS("scuc_data/scuc_enrollment_by_sex_acs1_table_b14002_labeled.rds")


# Save as CSV
write_csv(scuc_enrollment_by_sex_acs1_table_b14002_labeled, "scuc_data/scuc_enrollment_by_sex_acs1_table_b14002_labeled.csv")
...
write_xlsx(scuc_enrollment_by_sex_acs1_table_b14002_labeled, "scuc_data/scuc_enrollment_by_sex_acs1_table_b14002_labeled.xlsx")
...
## ACS1 TABLE B14001 - GRAND TOTALS
...
## ACS1 TABLE B14002 - TOTALS BY GRADE LEVELS
...
# 1. Load all ACS1 2023 variables
acs1_vars_2023 <- load_variables(2023, "acs1", cache = TRUE)

# 2. Extract the table names (prefix before the underscore + digits)
acs1_vars_2023 <- acs1_vars_2023 %>%
  mutate(table = str_extract(name, "^[A-Z0-9]+"))

table_list <- unique(acs1_vars_2023$table)
...
acs1_vars_schooldistricts <- acs1_vars_2023 %>%
  inner_join(availability %>% filter(available), by = "table") %>%
  select(name, label, concept, table)

acs1_vars_schooldistricts



# Save as RDS
saveRDS(acs1_vars_schooldistricts, "vars_tables/acs1_2023_vars_for_tx_schooldistricts.rds")

# # Load RDS later
# acs1_2023_vars_for_tx_schooldistricts <- readRDS("vars_tables/acs1_2023_vars_for_tx_schooldistricts.rds")


# # Save as RData (you can save multiple objects together)
# save(acs1_vars_schooldistricts, file = "vars_tables/acs1_2023_vars_for_tx_schooldistricts.RData")

# # Load later (restores object(s) into your workspace)
# load("vars_tables/acs1_2023_vars_for_tx_schooldistricts.RData")

# # Save as CSV
# write.csv(acs1_vars_schooldistricts, "vars_tables/acs1_2023_vars_for_tx_schooldistricts.csv", row.names = FALSE)

# # Load later
# acs1_2023_vars_for_tx_schooldistricts <- read.csv("vars_tables/acs1_2023_vars_for_tx_schooldistricts.csv")
...
# Define cache path
cache_file <- "vars_tables/acs1_2023_vars_for_tx_schooldistricts.rds"

# Check if cached file exists
if (file.exists(cache_file)) {
  message("Loading cached acs1_2023_vars_for_tx_schooldistricts...")
  acs1_2023_vars_for_tx_schooldistricts <- readRDS(cache_file)
} else {
  message("Building acs1_2023_vars_for_tx_schooldistricts and caching...")
  
  # --- Your code to build the object ---
  acs1_2023_vars_for_tx_schooldistricts <- acs1_vars_2023 %>%
    inner_join(availability %>% filter(available), by = "table") %>%
    select(name, label, concept, table)
  
  # Save to disk for future use
  saveRDS(acs1_2023_vars_for_tx_schooldistricts, "vars_tables/ACS1_2023_vars_for_tx_schooldistricts.rds", cache_file)
}
...
### Create Table of First Variable if Each ACS1 Table Available for School Districts
...
# File path for cache
cache_file <- "acs1_2023_firstvars_tx_schooldistricts.rds"

if (file.exists(cache_file)) {
  message("Loading cached version...")
  acs1_2023_firstvars_tx_schooldistricts <- readRDS(cache_file)
} else {
  message("Building and caching fresh version...")

  acs1_2023_firstvars_for_tx_schooldistricts <- acs1_vars_2023 %>%
    inner_join(availability %>% filter(available), by = "table") %>%
    filter(grepl("_001$", name)) %>%        # keep only first var in each table
    select(name, label, concept, table)

  # Save to disk for future use
  saveRDS(acs1_2023_firstvars_for_tx_schooldistricts, "vars_tables/acs1_2023_firstvars_for_tx_schooldistricts.rds")
}

# Inspect
head(acs1_2023_firstvars_for_tx_schooldistricts)
